...
boolean WHITE=0, right_flag=0, left_flag=0, plus=0, T=0, RF=0, LF=0, R=0, L=0, NODER=0, NODEL=0, SONAR=0, ALL_SENSOR=0;
...
void LFR_LINE_TRACK(){
  ...
  if(sum){
    ...
    if(total_error==0){
      left_speed=left_basespeed+100;
      right_speed=right_basespeed+100;
    }
    else{
    ...
    if(right_flag==0 && left_flag==0 && plus==0 && RF==0 && LF==0 && R==0 && L==0 && ALL_SENSOR==0){
      motor(100, 115);
      delay(50);
      sensor_read();
      sonar_check();
      while(!(ir[1]==1 || ir[2]==1 || ir[3]==1 || ir[4]==1 || ir[5]==1)){
        sonar_check();
        sensor_read();
        error=base_dis-dis;
        total_error=25.0*error+10.0*(error-previous_error);
        previous_error=error;
        left_speed=80-total_error;
        right_speed=95+total_error;
        motor(left_speed, right_speed);
      }
      ALL_SENSOR=0;
      SONAR=1;
...
void node_check(){
  if(ir[1]==0 && ir[2]==0 && ir[3]==0 && ir[4]==0 && ir[5]==0 && ir[6]==0 && ir[0]==0){
    ALL_SENSOR=1;
  }
...
void false_all_node(){
 WHITE=0; right_flag=0; left_flag=0; plus=0; RF=0; LF=0; R=0; L=0; NODER=0; NODEL=0;
}

void TURN(){
  if(WHITE==1){
    sensor_read();
    while( ir[0]==1 && ir[1]==1 && ir[5]==1 && ir[6]==1){
      motor(left_basespeed, right_basespeed);
    }
    motor(left_basespeed, right_basespeed);
    delay(50);
